!function() {
  Scene_Battle.prototype.addActorActionPromptLogText = function() {
    ...
    if (actor) {
      ...
      this.addLogCommandMessage("(ATQ: "+actor.atk+", DEF: "+actor.def+", VEL: "+actor.agi+", SUE: "+actor.luk+")", false);
    };
  };
  ...
  Sprite_EnemyBattlerStatus.prototype.refreshBitmap = function(battler) {
    ...
    if (battler) {
      ...
      // Get Battler State
      var EnemyEmotion = "";
      var SpecialEmotions = {119:"ENFADADO", 120:"ENFURECIDO", 121:"FURIOSO", 122:"EXTATICO", 123:"MANIACO", 124:"TRISTE", 125:"DEPRIMIDO", 126:"MISERABLE"};
      // Get States
      ...
      this.bitmap.drawText("ATQ: "+battler.atk, 8, 0, this.width, 90, 'left'); //Displays the enemy's ATTACK
      this.bitmap.drawText("DEF: "+battler.def, -8, 0, this.width, 90, 'right'); //Displays the enemy's DEFENSE
      this.bitmap.drawText("VEL: "+battler.agi, 8, 0, this.width, 120, 'left'); //Displays the enemy's SPEED
      this.bitmap.drawText("SUE: "+battler.luk, -8, 0, this.width, 120, 'right'); //Displays the enemy's LUCK
  ...
  Scene_Battle.prototype.pushPartyMessage = function() {
    ...
    if (BattleManager.canEscape()) {
      this.addLogCommandMessage("(Probabilidad de escapar: "+escapeRatio+"%)", false);
    }
